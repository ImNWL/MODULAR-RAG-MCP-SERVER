...
> Pick ONE test → Run ONE command → Wait for output → Record ONE row in `.cursor/skills/qa-tester/QA_TEST_PROGRESS.md` → THEN pick next.
...
> After completing a section, run `python .cursor/skills/qa-tester/scripts/qa_validate_notes.py`.
...
6. ⛔ GATE: Edit `.cursor/skills/qa-tester/QA_TEST_PROGRESS.md` (row + counters) — ONE row per edit
...
1. Read `.cursor/skills/qa-tester/QA_TEST_PLAN.md` for test steps and expected results.
2. Read `.cursor/skills/qa-tester/QA_TEST_PROGRESS.md` for current status.
...
```
Empty          → python .cursor/skills/qa-tester/scripts/qa_bootstrap.py clear
Baseline       → python .cursor/skills/qa-tester/scripts/qa_bootstrap.py baseline
DeepSeek       → python .cursor/skills/qa-tester/scripts/qa_config.py apply deepseek
Rerank_LLM     → python .cursor/skills/qa-tester/scripts/qa_config.py apply rerank_llm
NoVision       → python .cursor/skills/qa-tester/scripts/qa_config.py apply no_vision
InvalidKey     → python .cursor/skills/qa-tester/scripts/qa_config.py apply invalid_llm_key
InvalidEmbedKey→ python .cursor/skills/qa-tester/scripts/qa_config.py apply invalid_embed_key
Any            → no state change needed
```

After config-profile tests → `python .cursor/skills/qa-tester/scripts/qa_config.py restore`
Check state → `python .cursor/skills/qa-tester/scripts/qa_bootstrap.py status`
...
1. Read the test's **Steps** column from `.cursor/skills/qa-tester/QA_TEST_PLAN.md`.
...
```
python .cursor/skills/qa-tester/scripts/qa_multistep.py <TEST_ID>
```
...
For tests NOT in the script, run commands from `.cursor/skills/qa-tester/QA_TEST_PLAN.md` manually and paste output.
...
Edit `.cursor/skills/qa-tester/QA_TEST_PROGRESS.md`: update ONE test row + summary counters. ONE row per file edit.
...
```
python .cursor/skills/qa-tester/scripts/qa_validate_notes.py
```
...
| File | Purpose |
|------|---------|
| `.cursor/skills/qa-tester/QA_TEST_PLAN.md` | Test steps and expected results |
| `.cursor/skills/qa-tester/QA_TEST_PROGRESS.md` | Execution status and notes |
